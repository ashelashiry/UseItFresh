...
- Design/appearance questions about a page: FIRST call `ide.screenshot_canvas { screen: "<page name>" }` — it switches the Builder canvas there and returns a PNG. Critique what you see (don't ask permission); read code/theme only to explain it.
...
`mockApiWithStaticResponse`, `setMockDataSource`, `clearApiMock`,
...
Call a Postgres database function via Supabase RPC. Functions are synced from
the connected Supabase project (Settings -> Supabase -> Get Schema) onto
`currentSupabaseConfig.databaseConfig.rpcFunctions` — they cannot be authored
through the DSL. The builder takes the project so it can resolve the declared
signature at authoring time: unknown function names, unknown argument names,
and missing required arguments all throw `ArgumentError` immediately.
Arguments with a Postgres `DEFAULT` may be omitted (the generated call leaves
them out so Postgres applies the default). Requires Supabase to be enabled
(`app.supabase(...)`). When the function returns a value, the action's output
variable defaults to `<functionName>Result`.

```dart
Actions.callSupabaseRpc(project, functionName: 'log_visit');
Actions.callSupabaseRpc(
  project,
  functionName: 'compute_score',
  parameters: {'user_id': 'abc', 'weight': '0.5'},
