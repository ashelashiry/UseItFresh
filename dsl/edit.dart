...
/// Free and Plus (owner, 15 Sep): each account has a plan. On a free account
/// Plan my week, Your recipes and Receipts show a lock screen; Fits my goals,
/// Add to my week and shelf and receipt scanning open a Plus sheet; Shelf and
/// Receipt tiles carry a PLUS tag. Profile shows Your plan. Until migration
/// 10 runs the plan is 'open' and nothing is locked.
void buildStarterEditFlow(App app) {
  app.state('plan', string.withDefault('open'), persisted: true);
  app.raw((project) {
    ...
    if (!_uPlus()) {
      return _uPageBackground(SizedBox(
        ...
        child: SingleChildScrollView(
          ...
          child: _uPlusLock(context, 'Plan my week'),
        ),
      ));
    }
...
/// Plans (owner, 15 Sep): free or Plus. FFAppState().plan is 'plus', 'free'
/// or 'open' (the profile has no plan yet - migration 10 not run - so
/// nothing is locked).
  ...
  Widget build(BuildContext context) {
    ...
    if (!_uPlus()) {
      return _uPageBackground(SizedBox(
        ...
        child: SingleChildScrollView(
          padding: const EdgeInsets.fromLTRB(20, 12, 20, 32),
          child: _uPlusLock(context, 'Your recipe collection'),
        ),
      ));
    }
    final household = FFAppState().currentHouseholdId;
  ...
  Widget build(BuildContext context) {
    ...
    if (!_uPlus()) {
      return _uPageBackground(SizedBox(
        ...
        child: SingleChildScrollView(
          padding: const EdgeInsets.fromLTRB(20, 12, 20, 32),
          child: _uPlusLock(context, 'Receipt history'),
        ),
      ));
    }
    final household = FFAppState().currentHouseholdId;
            ...
            children: [
              ...
              Expanded(
                child: _UButton('Saved recipes',
                    kind: 'secondary',
                    icon: _uPlus() ? Icons.menu_book_outlined : Icons.lock_outline,
                    onTap: () => _uPlus()
...
class _SmartCameraState extends State<SmartCamera> with WidgetsBindingObserver {
  ...
  Future<void> _shoot() async {
    if (_mode != 'photo' && !_uPlus()) {
      _uPlusSheet(context, _mode == 'receipt' ? 'Receipt scanning' : 'Shelf scanning');
      return;
    }
    final c = _controller;
    if (_reading ||
  ...
  /// A mode button (owner, 15 Sep): an icon over its name, the chosen one
  /// cream with forest, the others frosted on the dark bar.
          ...
          decoration: BoxDecoration(
            gradient: on
                ? const LinearGradient(
                    ...
                    colors: [Color(0xFFFFFEFA), Color(0xFFE9EFE1)])
                : null,
            color: on ? null : const Color(0x1FFFFFFF),
            borderRadius: BorderRadius.circular(18),
            border: Border.all(
                color: on ? const Color(0xFFFFFFFF) : const Color(0x33FFFFFF)),
            boxShadow: on
                ? const [
                    BoxShadow(
...
class _AddOptionsState extends State<AddOptions> {
  ...
  Future<void> _shelf() async {
    if (!_uPlus()) {
      await _uPlusSheet(context, 'Shelf scanning');
      return;
    }
    await clearShelfScan();
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    Widget tile(String name, String purpose, Widget icon, List<Color> tone,
            Future<void> Function() go, {bool plus = false}) =>
        _USurface(
          tint: tone,
            ...
            child: Column(
              ...
              children: [
                ...
                Text(purpose,
                    ...
                    overflow: TextOverflow.ellipsis,
                    style: t.bodySmall
                        .copyWith(color: _uMuted, fontSize: 13, height: 1.3)),
              ],
            ),
          ),
        );
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          Text(
              'Scan a shelf, a barcode or a receipt. We suggest the foods and you check them before anything is added.',
              style: t.bodyLarge
                  .copyWith(color: _uMuted, fontSize: 15, height: 1.35)),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(
                child: tile(
                    'Shelf',
                    'Scan your fridge, freezer or pantry in seconds, a shelf at a time.',
              ...
              Expanded(
                child: tile(
                    'Barcode',
                    'Scan the code on a pack to fill in its name and photo.',
                    _svg(_barcode),
          ...
          Row(
            children: [
              Expanded(
                child: tile(
                    'Receipt',
                    'Photograph your shopping receipt to add the whole shop.',
                    _svg(_receipt),
                    const [Color(0xFFFFFDF6), Color(0xFFF0E4D0)], () async {
                  if (!_uPlus()) {
                    await _uPlusSheet(context, 'Receipt scanning');
                    return;
                  }
              ...
              Expanded(
                child: tile(
                    'Photo',
                    'Snap one food or package and we name it for you.',
                    _svg(_camera),
