...
/// Imperial units work: amounts on food cards, Use soon and a food's screen
/// show oz, lb and fl oz when My details is set to Imperial (food is still
/// stored in metric). Profile gets a header with the photo, name and email,
/// opening My details; the app remembers name, photo and units.
void buildStarterEditFlow(App app) {
  app.state('unitSystem', string.withDefault('metric'), persisted: true);
  app.state('profileName', string.withDefault(''), persisted: true);
  app.state('profileAvatar', string.withDefault(''), persisted: true);
  app.raw((project) {
    ...
    updateCustomWidget(project, name: 'MyDetails', code: _wMyDetails);
  });
}
...
/// Refreshes that copy from the profile (quietly; offline keeps the old one).
...
class _UseSoonListState extends State<UseSoonList> {
  ...
  Widget _row(FlutterFlowTheme t, Map<String, dynamic> item) {
    ...
    final amount = _uAmount(qty, (item['unit'] ?? '').toString()) ?? '';
            ...
            child: Column(
              ...
              children: [
                Text(name.isEmpty ? 'Add your name' : name,
                    ...
                    overflow: TextOverflow.ellipsis,
                    style: t.titleMedium.copyWith(
                        fontSize: 19, fontWeight: FontWeight.w800, color: _uInk)),
                if (email.isNotEmpty)
                  Text(email,
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
